...
class IsSupplierAddressInTheUkControllerSpec extends SpecBase with MockitoSugar {
  ...
  "IsSupplierAddressInTheUkController" - {
    ...
    "must return OK and the correct view for a GET when guard passes" in {
      ...
      running(application) {
        val request = FakeRequest(GET, isSupplierAddressInTheUkRoute)
        val result  = route(application, request).value
        val view    = application.injector.instanceOf[IsSupplierAddressInTheUkView]
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, isSupplierAddressInTheUkRoute)
    ...
    "must redirect to Unauthorised for a POST if no existing data is found" in {
      ...
      running(application) {
        val request = FakeRequest(POST, isSupplierAddressInTheUkRoute)
          .withFormUrlEncodedBody("value" -> "true")
        val result = route(application, request).value
    ...
    "must redirect to Unauthorised for a GET if the supplier number in the URL is not one of the user's suppliers" in {
      ...
      running(application) {
        val request = FakeRequest(GET, isSupplierAddressInTheUkSubmitRoute(SupplierNumber(2), NormalMode))
        val result  = route(application, request).value
    ...
    "must redirect to Unauthorised for a POST if the supplier number in the URL is not one of the user's suppliers" in {
      ...
      running(application) {
        val request = FakeRequest(POST, isSupplierAddressInTheUkSubmitRoute(SupplierNumber(2), NormalMode))
          .withFormUrlEncodedBody("value" -> "true")
        val result = route(application, request).value
